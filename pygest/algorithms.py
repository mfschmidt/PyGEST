...
class Algorithms(object):
    """ Implement common algorithms used on gene expression data
    """
    ...
    def corr_expr_conn(self, expr, conn, method=''):
        ...
        if isinstance(expr, pd.DataFrame) and isinstance(conn, pd.DataFrame):
            self._logger.debug("CEC: correlating two DataFrames, filtering and converting to arrays")
            ...
            self._logger.debug("CEC: expression & connectivity filtered down to {} matrices".format(expr_mat.shape))
            ...
            if isinstance(expr, np.ndarray):
                ...
                try:
                    if expr.shape[0] == expr.shape[1]:
                        self._logger.debug("expr is {}, assuming it's already a correlation matrix.".format(expr.shape))
                        final_expr_vector = expr[np.tril_indices(n=expr.shape[0], k=-1)]
                    else:
                        self._logger.debug("expr is {}, building a correlation matrix from it.".format(expr.shape))
                        expr_mat = np.corrcoef(expr, rowvar=False)
                        final_expr_vector = expr_mat[np.tril_indices(n=expr_mat.shape[0], k=-1)]
                except IndexError:
                    self._logger.debug("expr is {}, assuming it's already triangular vector.".format(expr.shape))
            ...
            if isinstance(conn, np.ndarray):
                ...
                try:
                    if conn.shape[0] == conn.shape[1]:
                        self._logger.debug("conn is {}, assuming it's a connectivity matrix.".format(conn.shape))
                    ...
                    self._logger.debug("conn is {}, assuming it's already a triangular vector.".format(conn.shape))
        ...
        self._logger.debug("CEC: {} {}-length expression vector with {}-length connectivity vector.".format(
