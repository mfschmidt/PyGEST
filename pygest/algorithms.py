...
def deprecated_order_probes_by_correlation_contribution(expr, conn, ascending=True, include_full=False, logger=None):
    """ Perform repeated correlations between versions of expr's correlation matrix and conn.
    ...
    # There are several ways to access the function name. I think all are ugly, so this works.
    f_name = 'order_probes_by_correlation_contribution'
    ...
    for p in expr.index:
        expr_mat = np.corrcoef(expr.drop(labels=p, axis=0), rowvar=False)
        expr_vec = expr_mat[np.tril_indices(n=expr_mat.shape[0], k=-1)]
        correlations.update({p: stats.pearsonr(expr_vec, conn_vec)[0]})

    elapsed = time.time() - full_start

    # Log results
    logger.debug("    ran scipy.stats.pearsonr {} times in 1 process (OPENBLAS_NUM_THREADS={}).".format(
        len(expr.index), BLAS_THREADS
    ))
    logger.info("    {} correlations in {:0.2f}s.".format(len(expr.index), elapsed))
    ...
    # TODO: This expensive DataFrame should be automatically cached for cheap retrieval later
    if include_full:
        return corr_df.sort_values(by='delta', ascending=ascending)
    else:
        return corr_df.sort_values(by='delta', ascending=ascending).drop(labels=0, axis=0)
...
def deprecated_whack_a_gene(expr, conn, method='', corr='', cores=0, logger=None):
    """ Perform repeated correlations between versions of expr's correlation matrix and conn.
    ...
    logger.info("whack_a_gene ran {} {} correlations with {} processes and OPENBLAS_NUM_THREADS={} in {:0.2f}s.".format(
        len(expr.index), corr, cores, BLAS_THREADS, elapsed
    ))
